...
def find_M_andComputeSmallestPair(left, right, minleft, minright):
    # L = len(left)-1
    # M = []
    # leftside = True
    # rightside = True
    # counter = len(leftside)
    # while leftside == True:
    #
    # # sorted = MergeSort()
    #
...
def findSmallest(array):
    ...
    for f in range(0, len(array)):
        for s in range(f, len(array)):
            if (not (f == s)):
                distance = distance(array[f], array[s])
                if distance == minimum:
...
def closestPair(array):
    ...
    if (len(array) <= 3):
       ...
       # return minimum
        ...
        # min is minimum of minleft and minright
